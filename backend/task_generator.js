...
function generateTasks(categories) {
    const order = categories.map(value => ({ value, sort: Math.random() }))
    ...
    const division_index = order.indexOf('division')
    if (division_index !== -1){
    ...
    const multiplication_index = order.indexOf('multiplication')
