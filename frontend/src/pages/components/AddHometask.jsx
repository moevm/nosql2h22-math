...
export default function AddHometask(){
    ...
    const displayHometask = () => <div>
        {tasks.map(task => <div key={task.id} className={styles.task} >
            ...
            {tasks.length > 1 ? /*<Button onClick={deleteTask(task.id)} style={{marginLeft: '5px'}}>Удалить</Button>*/
                <IconButton color="primary" aria-label="upload picture" component="label" onClick={deleteTask(task.id)}>
